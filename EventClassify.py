...
# 获取原文事件
def getEvents(loc):
    records = pd.read_csv(loc)
    tlv_cn = {}
    for index, row in records.iterrows():
        name = row['NAME']
        cont = [text.strip() for text in row['CONT'].split('\n')]
        tlv_cn[name] = cont
    return tlv_cn

# 聚类k
def sureK(embeddings, max_k, title):
    embedding = embeddings
    '利用SSE选择k'
    SSE = []  # 存放每次结果的误差平方和
    for k in range(1, max_k):
        estimator = KMeans(n_clusters=k, random_state=42, verbose=0)  # 构造聚类器
...
# 向量
def getEmbeddings(events):
    # 使用BERT模型生成文本的嵌入向量
    encoded_inputs = tokenizer(events, return_tensors='pt', padding=True, truncation=True, max_length=512)
    with torch.no_grad():
        outputs = model(**encoded_inputs)

    # 获取最后一层隐藏状态的平均值作为文本向量
    last_hidden_states = outputs.last_hidden_state
    embeddings = last_hidden_states.mean(dim=1).numpy()
    return embeddings
...
# 聚类
def getClassify(events, embeddings, k, key):
    # 保存原始的stdout
